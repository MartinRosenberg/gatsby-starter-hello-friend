...
First you need to clone this repo by `git clone https://github.com/MartinRosenberg/gatsby-starter-hello-friend.git` then switch to main dir, probably `cd gatsby-starter-hello-friend` should work fine. Next you should install all dependencies by `yarn` and then, to run starter on localhost `yarn dev` (which is `gatsby clean && gatsby develop`). If you want to build starter just run `yarn build` (which is `gatsby build`).
...
If you spot any bugs, please use the [issue tracker](https://github.com/MartinRosenberg/gatsby-starter-hello-friend/issues). If you want to add a new feature directly, please create a new [pull request](https://github.com/MartinRosenberg/gatsby-starter-hello-friend/pulls).
